...
public class Client {
	...
	public Client(ClientFrame clientFrame) {
		...
		//리스트
		userList = this.loungeFrame.getLoungePanel().getUserList();
		gameList = this.loungeFrame.getLoungePanel().getGameList();
		mainMessageBox = this.loungeFrame.getLoungePanel().getMessageFrame().getMessagePanel().getMainMessageBox();
		
		// 버튼
		SelectButton1 = this.loungeFrame.getLoungePanel().getSelectButton1();
		SelectButton2 = this.loungeFrame.getLoungePanel().getSelectButton2();
		SelectButton3 = this.loungeFrame.getLoungePanel().getSelectButton3();
		outRoomBtn = this.loungeFrame.getLoungePanel().getOutRoomBtn();
		sendMessageBtn = this.loungeFrame.getLoungePanel().getMessageFrame().getMessagePanel().getSendMessageBtn();
	}
...
//			outRoomBtn.setEnabled(false);
	...
	public void clickSendMessageBtn(String messageText) {
		if (myRoomName != null) {
			...
			String user = (String) loungeFrame.getLoungePanel().getUserList().getSelectedValue();
